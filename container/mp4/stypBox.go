...
func NewStypBox(b *Box) *StypBox {
	return &StypBox{
		Box: b,
	}
}

func (b *StypBox) Parse(r io.Reader) (totalReadLen int, err error) {

	if b.Size == 1 {
		err = fmt.Errorf("large size in styp box")
		return
	}
...
func (b *StypBox) Serialize(w io.Writer) (writedLen int, err error) {
